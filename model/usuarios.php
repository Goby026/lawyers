...
class Usuarios
{
    ...
    public function login1($usuario, $password)
    {
        ...
        $result = array();
        $stm = $this->pdo->prepare("SELECT * FROM ".$this->table." WHERE username = ? LIMIT 1");
...
//                $_SESSION['tipo_usuario'] = $login->id_tipo;
    ...
    public function usuarioExiste($usuario)
    {
        ...
        $stm = $this->pdo->prepare("SELECT idt_usuario FROM " . $this->table . " WHERE username = ? LIMIT 1");
        $stm->execute(array($usuario));
    ...
    {
        $stmt = $this->pdo->prepare("SELECT $campo FROM ".$this->table." WHERE $campoWhere = ? LIMIT 1");
    ...
    public function generaTokenPass($user_id)
    {
        ...
        $sql = "UPDATE ".$this->table." SET token_password=?, password_request=1 WHERE idt_usuario = ?";
    ...
    public function verificaTokenPass($user_id, $token)
    {
        ...
        $stm = $this->pdo->prepare("SELECT * FROM t_usuario WHERE idt_usuario = ? AND token_password = ? AND password_request = 1 LIMIT 1");
    ...
    public function cambiaPassword($password, $user_id, $token)
    {
        ...
        $sql = "UPDATE ".$this->table." SET password = ?, token_password='', password_request=0 WHERE idt_usuario = ? AND token_password = ?";
